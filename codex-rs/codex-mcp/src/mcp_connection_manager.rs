...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            emit_starting_update(startup_submit_id.as_str(), &tx_event, &server_name).await;
...
async fn emit_starting_update(submit_id: &str, tx_event: &Sender<Event>, server_name: &str) {
    let _ = emit_update(
        submit_id,
        tx_event,
...
async fn fetch_and_cache_tools_for_server(
    ...
    let tools =
        list_tools_for_client_uncached(server_name, client, timeout, server_instructions).await?;
    emit_duration(
...
fn github_mcp_personal_access_token_message(server_name: &str) -> String {
    format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = CODEX_GITHUB_PERSONAL_ACCESS_TOKEN"
    )
}

fn mcp_login_required_message(server_name: &str) -> String {
    format!(
        "The {server_name} MCP server is not logged in. Run `{APP_EXECUTABLE_NAME} mcp login {server_name}`."
    )
}

fn mcp_startup_timeout_message(server_name: &str, startup_timeout_secs: u64) -> String {
    format!(
