...
impl NetworkApprovalService {
    ...
    pub(crate) async fn record_blocked_request_owned(self: Arc<Self>, blocked: BlockedRequest) {
        self.record_blocked_request(blocked).await;
    }
    ...
    pub(crate) async fn handle_inline_policy_request_owned(
        self: Arc<Self>,
        session: Arc<Session>,
        request: NetworkPolicyRequest,
    ) -> NetworkDecision {
        self.handle_inline_policy_request(session, request).await
    }
}
    ...
    Arc::new(move |blocked: BlockedRequest| {
        ...
        async move {
            let _ = tokio::task::spawn_blocking(move || {
                ...
                handle.block_on(network_approval.record_blocked_request_owned(blocked))
            })
            .await;
        }
    })
    ...
    Arc::new(move |request: NetworkPolicyRequest| {
        ...
        async move {
            ...
            .await
            {
                ...
                Err(err) => {
                    warn!("network approval decider task failed: {err}");
