...
pub(crate) fn build_specs_with_discoverable_tools(
    ...
    let grok_spec = if config.code_mode_enabled {
        augment_tool_spec_for_code_mode(create_grok_tool())
    } else {
        create_grok_tool()
    };
    builder.push_spec(grok_spec);
...
fn create_grok_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "query".to_string(),
            JsonSchema::string(Some("Research question to answer with Grok.".to_string())),
        ),
        (
            "platform".to_string(),
