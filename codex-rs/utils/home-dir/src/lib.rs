...
impl ConfigNamespace {
    pub fn default_dir_name(self) -> &'static str {
        match self {
            Self::CodexCompatible => ".codex",
            Self::GodexIsolated => ".godex",
        }
    }

    pub fn env_var_name(self) -> &'static str {
        match self {
            Self::CodexCompatible => "CODEX_HOME",
            Self::GodexIsolated => "GODEX_HOME",
        }
    }
}
...
pub fn find_home(namespace: ConfigNamespace) -> std::io::Result<PathBuf> {
    let home_env = std::env::var(namespace.env_var_name())
        .ok()
        .filter(|val| !val.is_empty());
    find_home_from_env(namespace, home_env.as_deref())
}

fn find_home_from_env(
    namespace: ConfigNamespace,
    configured_home_env: Option<&str>,
) -> std::io::Result<PathBuf> {
    let env_var_name = namespace.env_var_name();
    let default_dir_name = namespace.default_dir_name();
    match configured_home_env {
        Some(val) => {
...
mod tests {
    ...
    fn find_codex_home_env_missing_path_is_fatal() {
        ...
        let err = find_home_from_env(ConfigNamespace::CodexCompatible, Some(missing_str))
    ...
    fn find_codex_home_env_file_path_is_fatal() {
        ...
        let err = find_home_from_env(ConfigNamespace::CodexCompatible, Some(file_str))
    ...
    fn find_codex_home_env_valid_directory_canonicalizes() {
        ...
        let resolved = find_home_from_env(ConfigNamespace::CodexCompatible, Some(temp_str))
