...
- `codex-rs/cli/src/mcp_cmd.rs`
  ...
  - remaining MVP residue: command-specific OAuth login/logout orchestration still lives inline
- `codex-rs/core/src/network_proxy_loader.rs`
...
- `codex-rs/core/src/mcp_connection_manager.rs`
  - startup/login copy, Codex Apps cache-context selection, cache read/startup snapshot access, and uncached tool refresh plus cache-write metrics glue are now adapterized
  - remaining MVP residue: startup event/status flow and other manager orchestration still live inline
- `codex-rs/tui/src/app.rs`
...
- `3. patch/bootstrap-proxy-mcp`, `patch/bootstrap-login-auth`, and `patch/bootstrap-runtime-ui`: keep the remaining residue in thin helper layers only; do not let deeper fork behavior move back into hot files.
