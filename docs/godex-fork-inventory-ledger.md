...
- Current recommendation: Keep the policy. The MVP extraction is now structurally closed; reopen this lane only for real upstream seam changes or additional wrapper-crate migration work.
- Notes:
  ...
  - `4a04f9e38c` advanced `patch/config-home-namespace` again by extracting root CLI namespace/override propagation into `codex-rs/cli/src/root_cli_policy.rs`, shrinking remaining fork-policy ownership in `cli/src/main.rs` around root config overrides, interactive `godex-home` propagation, and feature config-home resolution.
...
- Current recommendation: Keep. This is the right long-term direction for provider integration work, and the MVP contract extraction is now structurally closed; future work here should stay on external workers/config seams instead of reintroducing internal provider glue.
- Notes:
...
- `codex-rs/cli/src/login.rs` and `codex-rs/login/src/**`
  ...
  - device-code prompt copy, OAuth error copy, success/cancel body text, branded login error-page rendering, and branded success-page rendering now all start in `codex-rs/login/src/copy.rs`
  - the local ChatGPT auth handoff now starts in `codex-rs/tui/src/onboarding/auth/local_chatgpt_auth.rs` instead of a top-level TUI module
  - remaining MVP residue: deeper auth behavior still lives in `codex-rs/login/src/**`, but branded HTML assets are now template-driven and `login status` / `logout` are no longer tracked as MVP blockers
- `codex-rs/cli/src/mcp_cmd.rs`
  - config-home resolution and user-facing MCP copy are now adapterized through `mcp_copy.rs`
  - transport assembly, global MCP store write, and command-specific OAuth login/logout orchestration now start in dedicated helpers inside `codex-rs/cli/src/mcp_cmd.rs`
  - remaining MVP residue: no additional MVP blocker remains here; future work is sync-only maintenance
- `codex-rs/core/src/network_proxy_loader.rs`
  - loader bootstrap, config-layer source path resolution, trusted-layer policy assembly, and final state/constraint composition are now adapterized helpers
  - remaining MVP residue: no additional MVP blocker remains here; future work is sync-only maintenance
- `codex-rs/core/src/mcp_connection_manager.rs`
  - startup/login copy, Codex Apps cache-context selection, cache read/startup snapshot access, uncached tool refresh plus cache-write metrics glue, and startup event/status orchestration are now adapterized
  - remaining MVP residue: no additional MVP blocker remains here; future work is sync-only maintenance
- `codex-rs/tui/src/app.rs`
  - browser-open user copy now starts in `codex-rs/tui/src/runtime_ui_copy.rs`
  - startup update-notice and upstream-gap insertion now start in `codex-rs/tui/src/app/runtime_ui.rs`
  - remaining MVP residue: broader runtime presentation drift still needs sync-only review, but update/gap notice insertion is no longer an MVP blocker
...
| `patch/config-home-namespace` | `fork/config-namespace-home` | Keep `godex`/`godex -g` home behavior, config namespace policy, and schema exposure coherent. | `codex-rs/cli/src/main.rs`, `codex-rs/cli/src/root_cli_policy.rs`, `codex-rs/cli/tests/godex_home.rs`, `codex-rs/core/src/config/home_policy.rs`, `codex-rs/core/src/config/mod.rs`, `codex-rs/core/src/config_loader/**`, `codex-rs/utils/home-dir/src/lib.rs`, `codex-rs/core/config.schema.json`, `docs/config.md` | `cargo test -p codex-cli --test godex_home --manifest-path codex-rs/Cargo.toml -- --nocapture`; `cargo test -p codex-core home_policy --manifest-path codex-rs/Cargo.toml -- --nocapture`; `godex --memory-scope project --version` | Upstream exposes the same namespace/home policy hooks or the fork splits this into a dedicated wrapper crate |
...
- `3. patch/bootstrap-proxy-mcp`, `patch/bootstrap-login-auth`, and `patch/bootstrap-runtime-ui`: treat the MVP cut as structurally closed for now; reopen only for real sync conflicts, regressions, or newly exposed fork glue.
- `4. patch/backend-contract` and `patch/config-home-namespace`: treat both as MVP-closed structural lanes; reopen only for upstream seam changes, wrapper-crate extraction, or new external backend contract requirements.
