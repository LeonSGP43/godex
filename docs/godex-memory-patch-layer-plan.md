...
- `1710630a84 refactor(memory): extract rollout summary suffix contract`
- `1c2b2c1b2f refactor(memory): extract vector index metadata contract`
...
- Step 2 has partial progress: artifact-root, path-layout rules, empty-consolidation cleanup target ownership, rollout-summary file/path naming, rollout-summary suffix parsing, and vector-index metadata naming have started moving behind `fork_patch::memory`, but `core/src/memories/*` still owns parts of the contract.
