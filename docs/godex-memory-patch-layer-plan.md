...
- Step 2 has partial progress: artifact-root, path-layout rules, empty-consolidation cleanup target ownership, rollout-summary file/path naming, and rollout-summary suffix parsing have started moving behind `fork_patch::memory`, but `core/src/memories/*` still owns parts of the contract.
